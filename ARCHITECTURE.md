...
```
├── 대시보드          ← 홈. 시스템 상태 한눈에 보기
│   ├── 시즌 현황 / 광고 전략 모드 (네이버·쿠팡)
│   ├── 데이터 수집 상태 (마지막 분석 시각·상태)
│   ├── 검토 대기 제안 건수
│   └── 빠른 이동 버튼 (데이터 현황 / 제안함 / 분석)
│
├── 데이터 현황       ← API로 가져온 원본 데이터 확인
│   ├── 상품 목록     ← 네이버/쿠팡 상품 수집 현황
│   ├── 매출 내역     ← 상품별/플랫폼별 매출 집계
│   ├── 광고 현황     ← 캠페인/키워드/광고소재 (네이버)
│   └── 쿠팡 현황     ← 쿠팡 상품 + Wing 광고 현황
│
├── 분석              ← [분석 시작] 버튼으로 수집~제안 전체 실행
│   └── AI 종합 판단 요약 — 플랫폼별 섹션, "자세히 보기"로 펼치기 (탭 없음)
│
├── 제안함            ← AI 제안 카드형 UI (72시간 만료 룰)
│   ├── 대기 제안     ← PASS/WARN 판정 제안
│   ├── 승인/거절 이력
│   └── 관찰 필요 항목 ← Validator NEEDS_DATA 판정 제안 모음
│
├── 실행 로그         ← Action Log + 성과 verdict 확인
│   ├── 실행 이력
│   └── 성과 측정     ← Effect Tracker 결과 (7일 후 자동 측정)
│
└── 설정
...
**설계 원칙:**
- 대시보드: 읽기 전용, 상태 허브 — 수치 상세는 데이터 현황 메뉴에서
