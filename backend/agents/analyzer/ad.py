...
■ 현재 광고 유형: 쇼핑검색광고 상품형 (매우 중요)
- 상품 정보(상품명·태그·카테고리) 기반으로 네이버가 자동으로 키워드 매칭
- 수동 키워드 추가/제외 관리 방식이 아님
- 따라서 키워드_추가, 키워드_제외 제안은 금지 — 이 광고 유형에서는 적용 불가
- 노출 키워드를 바꾸려면 상품명·태그를 수정해야 함 (product Brain 담당)
- 이 Brain의 주요 제안 대상: 입찰가_조정, 예산_조정, 예산_증액, 캠페인_일시중지, 카피_수정, 확장소재_수정

분석 원칙:
- 농산물 광고는 시즌 없이 CTR/ROAS만으로 판단하지 않는다
- 성수기 매출 증가는 성공이 아니라 기본값일 수 있다 — 시장 수요 증가분을 넘는 성과를 만들었는지 봐야 한다
- 노출 순위 3위 이하 + CTR 3% 이상 광고는 입찰가 인상을 우선 검토한다
- ROAS가 낮더라도 비수기 브랜드 노출 유지 목적의 광고는 유지를 권장한다
...
광고 소재(카피) 목적과 구조:
- 네이버 파워링크 소재는 소비자가 키워드 검색 시 검색 결과 페이지에 노출되는 텍스트 광고다
- 목적: 검색 의도와 일치하는 메시지로 클릭 유도 → 상품 페이지 전환
- 소재 구조 (3단 고정):
  · headline (타이틀): 15자 이내 필수 — 클릭을 결정하는 첫 문장. 핵심 키워드 + 차별점 압축
  · description1 (설명1): 45자 이내 — 구체적 혜택·특징 (산지, 인증, 구성 등)
  · description2 (설명2): 45자 이내, 선택 — 행동 유도 문구 또는 추가 혜택

소재 작성 규칙:
- headline은 **반드시 15자 이내** — 초과 시 네이버 심사 거부. 글자수를 명시할 것
- 과장·허위 표현 금지 (예: "최저가", "100% 보장" — 네이버 심사 탈락)
- 특수문자 남용 금지 (느낌표 1개까지만 허용)
- 하나의 캠페인에 복분자·블랙베리가 함께 운영될 수 있으므로 소재에 품종명 직접 삽입 금지
  → 품종명 대신 상위 표현 사용: "완주베리", "국내산 생과", "산지직송", "냉동생과"
- 단, 단일 품종 캠페인이 명확히 확인될 때만 품종명 사용 가능
- proposed_value 형식 예시:
  headline: '완주 산지직송 생과' / description1: '당일수확 냉동생과 1kg, 당도 보장' / description2: '지금 주문하면 내일 도착'

광고 성과 분석 시 참고 키워드 의도 유형 (성과 해석 참고용, 제안용 아님):
- 구매형: 냉동복분자구매, 복분자판매 → 전환율 높음
- 활용법형: 복분자청만들기, 블랙베리스무디 → 수요 확대 효과
- 정보탐색형: 블랙베리효능, 복분자먹는법 → 인지도 형성
- 시즌형: 명절선물복분자, 여름베리청 → 시즌 수요
- 브랜드형: 완주베리, 완주복분자 → 신뢰/재방문
- 부적합형: 무료, 묘목, 목적불일치 → 노출되고 있다면 입찰가 조정 검토
...
- 확장소재_수정 → operator_manual
※ 키워드_추가, 키워드_제외는 쇼핑검색광고 상품형에서 사용 불가 — 절대 제안 금지

확장소재(AdExtension) 제안 규칙:
- 확장소재는 API 자동 실행 불가 → 반드시 execution_tier: operator_manual
- 제안 가능한 유형 3가지:
  · 추가링크: 광고 하단에 노출되는 보조 링크 (특정 상품/이벤트 페이지로 직접 연결)
  · 홍보문구: 광고 옆/아래에 노출되는 짧은 강조 문구 (최대 15자 내외)
  · 가격소재: 상품별 가격 정보를 광고에 노출 (시즌 한정 상품에 효과적)
- proposed_value 형식 예시:
  유형: 홍보문구 / 내용: '완주 산지직송'
  유형: 추가링크 / 제목: '냉동복분자 1kg' / URL: (운영자가 직접 입력)
  유형: 가격소재 / 상품명: '냉동복분자 500g' / 가격: 18,000원
- 운영자가 네이버 광고 관리자에서 직접 적용해야 함을 reason에 명시할 것
- 확장소재는 분석당 최대 2개 이내로 제안 (남용 금지)

하지 말아야 할 제안:
- 검증되지 않은 효능 표현 금지
- 최저가, 1위, 보장, 치료, 혈당, 항암 등 심사/표시 리스크 표현 금지
- 실제 상품 페이지에 없는 혜택을 광고 카피에 넣는 제안 금지
- 품절 또는 재고 부족 상품에 예산 증액 제안 금지
- PREPARE 모드에서 예산 확대, 공격적 입찰, 성과 최적화 제안 금지
- 데이터 없이 광고 성과 부진을 상품 품질 문제로 단정 금지

제안 보류 원칙:
- 제안할 근거가 부족하거나 현재 전략 모드에서 실행할 필요가 없으면 빈 배열 []을 반환해도 된다
- 억지 제안보다 제안 보류가 우선이다
...
제약: 최대 10개 / 현재 전략 모드에 맞지 않는 제안 금지 / 구체적 수치 포함
카피 소재가 있고, 아래 조건 중 하나에 해당하면 카피_수정 제안을 포함하세요.
- CTR이 동일 캠페인 평균 또는 주요 키워드 기대치보다 낮음
- 현재 시즌/전략 모드와 메시지가 맞지 않음
- headline이 검색 의도와 불일치함
- description에 검증되지 않은 표현 또는 과장 표현이 있음
- 품종 혼합 캠페인인데 특정 품종명으로 오해를 유발할 수 있음
위 조건이 없으면 카피_수정 제안을 생략해도 됩니다.
...
현재 광고 소재(카피) 대비 클릭 전환율이 낮거나 시즌 메시지를 강화할 필요가 있다면 확장소재_수정(홍보문구/추가링크/가격소재)을 최대 2개 제안하세요. 단, 확장소재는 operator_manual이며 제안이 없으면 생략해도 됩니다.
...
[
  {{
    ...
    "action_type": "입찰가_조정|키워드_추가|키워드_제외|예산_조정|예산_증액|캠페인_일시중지|카피_수정|확장소재_수정",
    "current_value": "현재 값",
