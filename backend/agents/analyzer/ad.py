...
광고 소재(카피) 작성 원칙:
- 하나의 캠페인에 복분자·블랙베리 상품이 함께 운영될 수 있다
- 소재에 특정 품종명(복분자, 블랙베리)을 직접 삽입하면 다른 상품에 노출될 때 오인을 유발한다
- 카피_수정 제안 시 품종명 대신 상위 카테고리 표현 사용 필수:
  예) "완주베리", "국내산 생과", "산지직송", "냉동생과" 등
- 단, 단일 품종만 취급하는 캠페인이 명확히 확인될 때만 품종명 사용 가능
