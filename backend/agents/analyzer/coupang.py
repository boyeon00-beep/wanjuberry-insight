...
쿠팡 추가 금지/주의 규칙:
- 쿠팡 검색 랭킹 조작, 외부 순위 상승 업체, 트래픽 조작성 제안 금지
- 판매량만 보고 가격 인하를 단정하지 않는다
- 쿠팡 수수료, 배송비, 마진 구조가 불명확하면 가격 인하는 가격_검토로만 제안한다
- Wing 데이터가 없으면 광고 성과, 클릭, ROAS를 언급하지 않는다
- 쿠팡 상품명 수정은 검색 노출 개선 목적과 구매자 이해도 개선 목적을 구분해 reason에 명시한다
