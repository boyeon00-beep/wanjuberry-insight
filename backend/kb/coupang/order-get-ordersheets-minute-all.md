...
>
> 발주서 목록을 24시간 이내의 분단위 구간으로 조회합니다. 예) (2022-08-10T00:00 ~ 2022-08-10T23:59)
...
| Name | Required | Type | Description |
|------|----------|------|-------------|
| createdAtFrom | O | String | 검색 시작일시 (ISO-8601 준수) — yyyy-mm-ddT00:00%2B09:00 형태. 예) 2025-07-29T01:23%2B09:00 |
| createdAtTo | O | String | 검색 종료일시 (ISO-8601 준수) — yyyy-mm-ddT00:00 형태. 예) 2025-07-29T15:50%2B09:00. 해당 일자 내 24시간 이내로 조회 가능 |
| status | O | String | 발주서 상태 (Enum 참고) |
| searchType | | String | search type for order sheets results. searchType=timeFrame이면 발주서 목록 조회(분단위 전체)로 수행되며, 그 외에는 발주서 목록 조회(일단위 페이징)으로 수행됩니다. |
...
| &nbsp;&nbsp;&nbsp;&nbsp;invoiceNumberUploadDate | String | 운송장번호 업로드 일시 (ISO-8601, optional) — YYYY-MM-DDThh:mm:ss.ssssss±hh:mm |
      ...
      "orderItems": [
        {
          ...
          "shippingCount": 1,
          "salesPrice": { "currencyCode": "KRW", "units": 49900, "nanos": 0 },
          "orderPrice": { "currencyCode": "KRW", "units": 49900, "nanos": 0 },
...
| 400 (요청변수확인) | endTime-startTime range should less than 0 day | 조회기간이 24시간 이내인지 확인합니다. |
...
- createdAtFrom / createdAtTo는 ISO-8601 표준 준수, 날짜+시각 포함한 "yyyy-mm-ddT00:00%2B09:00" 형태로 입력
- 조회 기간은 최대 24시간 이내 (24시간 초과 시 에러 발생)
- searchType=timeFrame 파라미터를 반드시 포함해야 분단위 전체 조회로 동작
- 결제완료(ACCEPT) 상태에서 고객이 배송지를 변경할 수 있으므로, 상품준비중 처리 이후 발주서 단건 조회로 receiver 배송지 정보 재확인 필수
- 출고 전 "sellerProductName + sellerProductItemName"과 "vendorItemName" 정보 일치 여부 반드시 확인
- 구성/수량/용량 등 오노출 확인 시 출고 보류 후 온라인 문의 접수
- 반품완료건은 이 API로 조회 불가 — '반품/취소 요청 목록 조회' API 사용
